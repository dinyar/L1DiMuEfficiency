...
void triggerMuRates(const char* file_list, TString folder = "tmp",
                 TString run = "XXX", int nCollBunches = 2028,
                 bool retrieve_hists = false, bool update_hists = false,
                 int nEntries = 0) {
  TString plotFolder = "l1RatePlots/" + run + "/" + folder + "/";
  mkdir("l1RatePlots/", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  mkdir("l1RatePlots/" + run, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  mkdir(plotFolder, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  TString histFolder = "l1RateHists/" + run + "/" + folder + "/";
  mkdir("l1RateHists/", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  mkdir("l1RateHists/" + run, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  ...
  TChain* chainL1 = new TChain(l1Treepath.c_str());

  if (!retrieve_hists) {
    std::vector<std::string> listNtuples;

    bool success = readFList(file_list, listNtuples, l1Treepath);
    ...
    nEntries = setupTChain(listNtuples, chainL1);
  }
  ...
  getMuonRates(nCollBunches, nEntries, chainL1, singleMuRatesOpenVsPt,
  ...
  singleMuNoCutDesc << "Single Muon, q>7 rate";
  std::ostringstream doubleMuNoCutDesc;
  doubleMuNoCutDesc << "Double Muon, q>7 rate";
  drawHistograms(singleMuRatesOpenVsPt, doubleMuRatesOpenVsPt,
                 "openRatesVsPtMu", "p_{T} threshold [GeV]", "ZeroBias",
...
void drawHistograms(TH1D& singleMuHist, TH1D& doubleMuHist, TString filename,
  ...
  TLegend leg1(0.51, 0.72, 0.9, 0.92);
  leg1.SetFillColor(0);
  ...
  leg1.SetFillStyle(0);
  leg1.Draw();
  n1.DrawLatex(0.51, 0.68, "Runs " + run + ", #sqrt{s} = 13 TeV");
  n2.DrawLatex(0.51, 0.63, descString);
  ...
  ratioplot->GetYaxis()->SetLabelSize(17);
  ...
  ratioplot->GetXaxis()->SetLabelFont(
      43);  // Absolute font size in pixel (precision 3)
  ratioplot->GetXaxis()->SetLabelSize(17);
