...
class GMTDiMuEfficiency : public L1Ntuple
{
  ...
  TNtuple *ntuple;
  void toggleBranches();
  void fillNtuple(int recoMu1, int recoMu2, int gmtMu1, int gmtMu2, std::pair<bool, bool> diMuMatch, std::vector<std::string> contDict, Float_t ntupleValues[]);
  ...
  double dpt(int iRecoMu, int iL1Mu, int iL1Sys); // calculate delta pt between iRecoMu muon and iL1Mu trigcand of type iL1Sys
  ...
  std::pair<bool, bool> matchDiMuons(int iRecoMu1, int iRecoMu2, int &L1Mu1, int &L1Mu2, int iL1Sys, float ptcut, float dRmax); // finds the best two matches (lowest dR=dR1+dR2) for the two iRecoMus.
  ...
  for (Long64_t i=0; i<nevents; i++) {
    ...
    for(int recoMu1 = 0; recoMu1 < recoMuon_->nMuons; ++recoMu1) {
      ...
      for(int recoMu2 = recoMu1+1; recoMu2 < recoMuon_->nMuons; ++recoMu2) {
        ...
        int gmtMu1, gmtMu2;
        std::pair<bool, bool> diMuMatch = matchDiMuons(recoMu1, recoMu2, gmtMu1, gmtMu2, GMT, 0, 0.3);
